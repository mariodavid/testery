package de.diedavids.testery.web.metadata
...
@Component('testery_MetadataSelector')
@CompileStatic
class MetadataSelector {
    ...
    Map<String, Object> getEntitiesLookupFieldOptions() {
        getEntitiesLookupFieldOptions(Entity.class)
    }

    Map<String, Object> getEntitiesLookupFieldOptions(Class baseEntityClass) {
        Map<String, Object> options = new TreeMap<>()

        for (MetaClass metaClass : metadataTools.allPersistentMetaClasses) {
            if (readPermitted(metaClass)) {
                Class javaClass = metaClass.javaClass
                if (baseEntityClass.isAssignableFrom(javaClass)) {
                    options.put(messageTools.getEntityCaption(metaClass), metaClass.name)
                }
            }
        }
